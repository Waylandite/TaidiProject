...
def dataProcessing():
    #读取官方原始数据
    train1DF = pd.read_csv('../数据/官方数据_完整版/order_train2.csv', encoding='utf-8')
    #构造天数排序范围
    #Days对应的天数,2015年9月1日定义为Days=1，也就是第一天
    dateList = pd.date_range(start="20150901", end="20190320", freq="D")
    daysList = {}
    i = 1
    ...
    train1DF.to_csv('../数据/过程中数据2/EncodedTrainData_2015_201903.csv', index=False, encoding='utf-8')


#构造出2019年1月的数据，并且将数据编码后合合并保存
def addDataOf201904():
    #读取官方文件的2019年1月数据predict_sku1.csv
    sku1DF = pd.read_csv('../数据/官方数据_完整版/predict_sku2.csv', encoding='utf-8')
    ...
    train15_18DF = pd.read_csv('../数据/过程中数据2/EncodedTrainData_2015_201903.csv', encoding='utf-8')
    ...
    dateList = pd.date_range(start="20190321", end="20190430", freq="D")
    dates = []
    demandList = []
    combinationList = []
    for d in dateList:
    ...
    #构造2019年1月数据的天数列
    dateList = pd.date_range(start="20150901", end="20190430", freq="D")
    daysList = {}
    i = 1
    for date in dateList:
        daysList[str(date).split(' ')[0]] = i
    ...
    # 保存数据到train201901文件
    train201901DF.to_csv('../数据/过程中数据2/EncodedTrainData_201904.csv', index=False, encoding='utf-8')
    ...
    train2015_201901DF.to_csv('../数据/过程中数据2/EncodedTrainData_2015_201904.csv', index=False, encoding='utf-8')

#对2015年到2019年01月编码处理后的数据进行特征工程
def featureEngineeringOf2015_201904():
    # 读取2015年到2019年01月编码处理后的数据
    train15_1901DF = pd.read_csv('../数据/过程中数据2/EncodedTrainData_2015_201904.csv', encoding='utf-8')
    ...
    featureedTrain15_1901DF.to_pickle('../数据/过程中数据2/FeaturedEncodedTrainData_2015_201904.pkl')
    featureedTrain15_1901DF.to_csv('../数据/过程中数据2/FeaturedEncodedTrainData_2015_201904.csv', index=False, encoding='utf-8')


#构建模型为训练集和测试集用来预测2019年1月份的销售量
#读取编码以及特征工程后的2015年1月到2018年月的数据
def ModelPredict201904():
    # 读取编码以及特征工程后的2015年1月到2018年月的数据
    Train15_1901DF = pd.read_csv('../数据/过程中数据2/FeaturedEncodedTrainData_2015_201904.csv', encoding='utf-8')
    Train15_1901DF.drop('item_price', axis=1, inplace=True)
    # 划分验证集和测试集
    valid = Train15_1901DF[(Train15_1901DF['days_range'] >= 1269) & (Train15_1901DF['days_range'] < 1298)][['item_code', 'days_range', 'ord_qty']]
    test = Train15_1901DF[(Train15_1901DF['days_range'] >= 1298)][['item_code', 'days_range', 'ord_qty']]
    # 测试集训练预测和验证集预测结果
    ...
    for region in regionList:
        try:
            ...
            # 划分训练集和验证集
            X_train, y_train = regionedTrain15_1901DF[regionedTrain15_1901DF['days_range'] < 1269].drop('ord_qty', axis=1), \
                               regionedTrain15_1901DF[regionedTrain15_1901DF['days_range'] < 1269]['ord_qty']
            X_valid, y_valid = regionedTrain15_1901DF[(regionedTrain15_1901DF['days_range'] < 1298)&(regionedTrain15_1901DF['days_range'] >=1269)].drop('ord_qty', axis=1), \
                               regionedTrain15_1901DF[(regionedTrain15_1901DF['days_range'] < 1298)&(regionedTrain15_1901DF['days_range'] >=1269)]['ord_qty']
            X_test= regionedTrain15_1901DF[regionedTrain15_1901DF['days_range'] >= 1298].drop('ord_qty', axis=1)
            # 构建模型并模型
            model = LGBMRegressor(
                n_estimators=1000,
                learning_rate=0.1,
    ...
    train201901DF = pd.read_csv('../数据/过程中数据2/EncodedTrainData_201904.csv')
    train201901DF['ord_qty'] = result  # 把预测的订单需求量替换
    train201901DF.to_csv('../数据/结果数据2/ForecastedEncodedTrainData_201904.csv', index=False)
    #将预测后的2019年1月的数据合并到官方存在的的2015年1月到2018年12月的数据中
    tain2015_2018DF = pd.read_csv('../数据/过程中数据2/EncodedTrainData_2015_201903.csv')
    PredictedEncodedTrainData_2015_201901DF = pd.concat([tain2015_2018DF, train201901DF], axis=0)
    PredictedEncodedTrainData_2015_201901DF.to_csv('../数据/过程中数据2/PredictedEncodedTrainData_2015_201904.csv', index=False)
    #计算出一月总销售额
    forecasted201901DF = train201901DF.loc[train201901DF['days_range'] >= 1309]  # 定位到19年4月的数据
...
def addDataOf201905():
    ...
    train15_1901DF = pd.read_csv('../数据/过程中数据2/PredictedEncodedTrainData_2015_201904.csv', encoding='utf-8')
    ...
    dateList = pd.date_range(start="20190501", end="20190531", freq="D")
    dates = []
    demandList = []
    combinationList = []
    for d in dateList:
    ...
    #构造2019年2月数据的天数列
    dateList = pd.date_range(start="20150901", end="20190531", freq="D")
    daysList = {}
    i = 1
    for date in dateList:
        daysList[str(date).split(' ')[0]] = i
    ...
    # 保存数据到train201901文件
    train201902DF.to_csv('../数据/过程中数据2/EncodedTrainData_201905.csv', index=False, encoding='utf-8')
    #拼接2015年到2018年的真实数据和201901年自己构造的待预测数据
    train2015_201901DF=pd.concat([train15_1901DF,train201902DF],axis=0)
    train2015_201901DF.drop(['combination'], axis=1, inplace=True)
    train2015_201901DF.to_csv('../数据/过程中数据2/EncodedTrainData_2015_201905.csv', index=False, encoding='utf-8')
#对2015年到2019年02月编码处理后的数据进行特征工程
def featureEngineeringOf2015_201905():
    # 读取2015年到2019年01月编码处理后的数据
    train15_1902DF = pd.read_csv('../数据/过程中数据2/EncodedTrainData_2015_201905.csv', encoding='utf-8')
    ...
    featureedTrain15_1902DF.to_pickle('../数据/过程中数据2/FeaturedEncodedTrainData_2015_201905.pkl')
    featureedTrain15_1902DF.to_csv('../数据/过程中数据2/FeaturedEncodedTrainData_2015_201905.csv', index=False, encoding='utf-8')

#读取编码以及特征工程后的2015年1月到2018年月的数据
#为什么这一块的数据，从1207开始，就没有了，然后突然到1250才出现结果？
def ModelPredict201905():
    # 读取编码以及特征工程后的2015年1月到2018年月的数据
    Train15_1902DF = pd.read_csv('../数据/过程中数据2/FeaturedEncodedTrainData_2015_201905.csv', encoding='utf-8')
    Train15_1902DF.drop('item_price', axis=1, inplace=True)
    # 划分验证集和测试集
    valid=Train15_1902DF[(Train15_1902DF['days_range']<1339)&(Train15_1902DF['days_range'] >=1298)][['item_code', 'days_range', 'ord_qty']]
    test = Train15_1902DF[(Train15_1902DF['days_range'] >= 1339)][['item_code', 'days_range', 'ord_qty']]
    # 测试集训练预测和验证集预测结果
    testResult = test['ord_qty']  # 这是已有真实标签需求量1175到1206，31天间隔的真实数据
    ...
    for region in regionList:
        #try:
            ...
            # 划分训练集和验证集
            X_train, y_train = regionedTrain15_1902DF[regionedTrain15_1902DF['days_range'] < 1298].drop('ord_qty', axis=1), \
                               regionedTrain15_1902DF[regionedTrain15_1902DF['days_range'] < 1298]['ord_qty']
            X_valid, y_valid = regionedTrain15_1902DF[(regionedTrain15_1902DF['days_range'] < 1339)&(regionedTrain15_1902DF['days_range'] >= 1298)].drop('ord_qty', axis=1), \
                               regionedTrain15_1902DF[(regionedTrain15_1902DF['days_range'] < 1339)&(regionedTrain15_1902DF['days_range'] >= 1298)]['ord_qty']
            X_test = regionedTrain15_1902DF[regionedTrain15_1902DF['days_range'] >= 1339].drop('ord_qty', axis=1)
            # 构建模型并模型
            model = LGBMRegressor(
                n_estimators=1000,
                learning_rate=0.01,
    ...
    for i in testResult:
        if i < 0:
            ...
            result.append(round(i, 0))
    #读取之前构造好的19年1月数据,将预测的订单需求量替换
    train201902DF = pd.read_csv('../数据/过程中数据2/EncodedTrainData_201905.csv')
    train201902DF['ord_qty'] = result  # 把预测的订单需求量替换
    train201902DF.to_csv('../数据/结果数据2/ForecastedEncodedTrainData_201905.csv', index=False)
    #将预测后的2019年1月的数据合并到官方存在的的2015年1月到2018年12月的数据中
    tain2015_2018DF = pd.read_csv('../数据/过程中数据2/PredictedEncodedTrainData_2015_201904.csv')
    PredictedEncodedTrainData_2015_201901DF = pd.concat([tain2015_2018DF, train201902DF], axis=0)
    PredictedEncodedTrainData_2015_201901DF.to_csv('../数据/过程中数据2/PredictedEncodedTrainData_2015_201905.csv', index=False)
    #计算出而月总销售额
    forecasted201902DF = train201902DF.loc[train201902DF['days_range'] >= 1339]  # 定位到19年5月的数据
    ...
    predictDF.columns = ['sales_region_code', 'item_code', 'first_cate_code', 'second_cate_code']
    s = pd.merge(predictDF, resultDF, how='left')
    s.to_csv('../数据/结果数据2/ForecastedData_201905.csv', index=False)
...
def addDataOf201906():
    ...
    train15_1903DF = pd.read_csv('../数据/过程中数据2/PredictedEncodedTrainData_2015_201905.csv', encoding='utf-8')
    ...
    dateList = pd.date_range(start="20190601", end="20190630", freq="D")
    dates = []
    demandList = []
    combinationList = []
    for d in dateList:
    ...
    #构造2019年3月数据的天数列
    dateList = pd.date_range(start="20150901", end="20190630", freq="D")
    daysList = {}
    i = 1
    for date in dateList:
        daysList[str(date).split(' ')[0]] = i
    ...
    # 保存数据到train201901文件
    train201903DF.to_csv('../数据/过程中数据2/EncodedTrainData_201906.csv', index=False, encoding='utf-8')
    #拼接2015年到2018年的真实数据和201901年自己构造的待预测数据
    train2015_201901DF=pd.concat([train15_1903DF,train201903DF],axis=0)
    train2015_201901DF.drop(['combination'], axis=1, inplace=True)
    train2015_201901DF.to_csv('../数据/过程中数据2/EncodedTrainData_2015_201906.csv', index=False, encoding='utf-8')
#对2015年到2019年03月编码处理后的数据进行特征工程
def featureEngineeringOf2015_201906():
    # 读取2015年到2019年01月编码处理后的数据
    train15_1903DF = pd.read_csv('../数据/过程中数据2/EncodedTrainData_2015_201906.csv', encoding='utf-8')
    ...
    featureedTrain15_1903DF.to_pickle('../数据/过程中数据2/FeaturedEncodedTrainData_2015_201906.pkl')
    featureedTrain15_1903DF.to_csv('../数据/过程中数据2/FeaturedEncodedTrainData_2015_201906.csv', index=False, encoding='utf-8')


#读取编码以及特征工程后的2015年1月到2018年月的数据
def ModelPredict201906():
    # 读取编码以及特征工程后的2015年1月到2018年月的数据
    Train15_1903DF = pd.read_csv('../数据/过程中数据2/FeaturedEncodedTrainData_2015_201906.csv', encoding='utf-8')
    Train15_1903DF.drop('item_price', axis=1, inplace=True)
    # 划分验证集和测试集
    valid = Train15_1903DF[(Train15_1903DF['days_range'] >= 1309) & (Train15_1903DF['days_range'] < 1370)][['item_code', 'days_range', 'ord_qty']]
    test = Train15_1903DF[(Train15_1903DF['days_range'] >= 1370)][['item_code', 'days_range', 'ord_qty']]
    # 测试集训练预测和验证集预测结果
    ...
    for region in regionList:
        try:
            ...
            # 划分训练集和验证集
            X_train, y_train = regionedTrain15_1903DF[regionedTrain15_1903DF['days_range'] < 1298].drop('ord_qty', axis=1), \
                               regionedTrain15_1903DF[regionedTrain15_1903DF['days_range'] < 1298]['ord_qty']
            X_valid, y_valid = regionedTrain15_1903DF[(regionedTrain15_1903DF['days_range'] < 1370)&(regionedTrain15_1903DF['days_range'] >= 1309)].drop('ord_qty', axis=1), \
                               regionedTrain15_1903DF[(regionedTrain15_1903DF['days_range'] < 1370)&(regionedTrain15_1903DF['days_range'] >= 1309)]['ord_qty']
            X_test = regionedTrain15_1903DF[(regionedTrain15_1903DF['days_range'] >= 1370)].drop('ord_qty', axis=1)
            # 构建模型并模型
            model = LGBMRegressor(
                n_estimators=1000,
                learning_rate=0.01,
    ...
    for i in testResult:
        if i < 0:
            ...
            result.append(round(i, 0))
    #读取之前构造好的19年1月数据,将预测的订单需求量替换
    train201903DF = pd.read_csv('../数据/过程中数据2/EncodedTrainData_201906.csv')
    train201903DF['ord_qty'] = result  # 把预测的订单需求量替换
    train201903DF.to_csv('../数据/结果数据2/ForecastedEncodedTrainData_201906.csv', index=False)
    #将预测后的2019年1月的数据合并到官方存在的的2015年1月到2018年12月的数据中
    tain2015_2018DF = pd.read_csv('../数据/过程中数据2/PredictedEncodedTrainData_2015_201905.csv')
    PredictedEncodedTrainData_2015_201901DF = pd.concat([tain2015_2018DF, train201903DF], axis=0)
    PredictedEncodedTrainData_2015_201901DF.to_csv('../数据/过程中数据2/PredictedEncodedTrainData_2015_201906.csv', index=False)
    #计算出而月总销售额
    forecasted201903DF = train201903DF.loc[train201903DF['days_range'] >= 1370]  # 定位到19年3月的数据
    ...
    s.to_csv('../数据/结果数据2/ForecastedData_201906.csv', index=False)
#合并1月2月3月的数据
def Merge():
    Precited201901DF = pd.read_csv('../数据/结果数据2/ForecastedData_201904.csv')
    Precited201902DF = pd.read_csv('../数据/结果数据2/ForecastedData_201905.csv')
    Precited201903DF = pd.read_csv('../数据/结果数据2/ForecastedData_201906.csv')
    ...
    submit_df.columns = ['sales_region_code', 'item_code', '2019年4月预测需求量', '2019年5月预测需求量', '2019年6月预测需求量']
    # 保存
    submit_df.to_excel('../数据/结果数据2/result2.xlsx', index=False)

if __name__ == '__main__':
    dataProcessing()
    addDataOf201904()
    featureEngineeringOf2015_201904()
    ModelPredict201904()
    addDataOf201905()
    featureEngineeringOf2015_201905()
    ModelPredict201905()
    addDataOf201906()
    featureEngineeringOf2015_201906()
    ModelPredict201906()
    Merge()
    pass
